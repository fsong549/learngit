...
//add by fpsensor lzk 2016.06.13
...
FingerprintDaemonProxy::FingerprintDaemonProxy() : mModule(NULL), mDevice(NULL), mCallback(NULL) {
    ALOGD(FPTAG"FingerprintDaemonProxy invoked");
}
...
int64_t FingerprintDaemonProxy::openHal() {
    ...
    for(int i = 0; i < FP_VARIANT_KEYS_COUNT; i++) {
        ...
        if (NULL == hw_module) {
            ALOGE(FPTAG"No such a fingerprint module:%s.", variant_keys[i]);
            continue;
        }
        ...
        if (0 != (err = mModule->common.methods->open(hw_module, NULL, &device))) {
            ALOGE(FPTAG"Can't open fingerprint methods, error: %d", err);
            continue;
        }
        ...
        if (err < 0) {
            ALOGE(FPTAG"Failed in call to set_notify(), err=%d", err);
            continue;
        }
        // Sanity check - remove
        if (mDevice->notify != hal_notify_callback) {
            ALOGE(FPTAG"NOTIFY not set properly: %p != %p", mDevice->notify, hal_notify_callback);
        }
        ALOGD(FPTAG"fingerprint HAL successfully initialized,cur sensor = %s",variant_keys[i]);
        break;
    }

    if (mDevice == NULL) {
        ALOGE(FPTAG"fingerprint HAL failed to initialized");
    }
